...
$.removeCookie("assetsPlanningPrefsData");
$.removeCookie("typesPrefsData");
$.removeCookie("assetProfilesData");
var totalImageSize = 0;

var assetProfilesData = {'filters': JSON.stringify({"groupOp":"AND","rules":[{"field":"asset_profiles.deleted_at","op":"eq","data":null}]}), _search: false, rows: 20, page: 1, sidx: "title", sord: "asc"};
...
$('#profileTitle').on('change', function() {
...
var assetprofiles = function() {
        ...
        $(document).on('change','#category_id',function(e){
         ...
         //    })
            if($('#category_id').select2('data').text.toLowerCase()=="trailers"){
                ...
                $('.js-adr-test-interval').removeClass('d-none');
                hideShowRegularCheckFrequency('hide');
                $('#odometer_type').select2("val", "na");
                $('#fuel_type').select2("val", "na");
                $('#engine_type').select2("val", "na");
                $('.js-rubber-integrity-test-interval').removeClass('d-none');
                $('.js-loler-test-interval').addClass('d-none');
                $('.js-electrical-inspection-interval').addClass('d-none');
            }else{
                $('.js-pmi-interval').addClass('d-none');
                $('.js-adr-test-interval').addClass('d-none');
                hideShowRegularCheckFrequency('show');
                $('#odometer_type').select2("val", "");
                $('#fuel_type').select2("val", "");
                $('#engine_type').select2("val", "");
                $('.js-rubber-integrity-test-interval').addClass('d-none');
                $('.js-loler-test-interval').removeClass('d-none');
                $('.js-electrical-inspection-interval').removeClass('d-none');
            }
        });
    ...
    function hideShowRegularCheckFrequency(action='show'){
        if(action=='show'){
            $("#tank_test_interval").parent('div').parent('div').find('label').hide();
            $("#tank_test_interval").parent('div').parent('div').find('label').next('div').hide();
            ...
            $("#tank_test_interval").parent('div').parent('div').find('label').show();
            $("#tank_test_interval").parent('div').parent('div').find('label').next('div').show();
    ...
    var validateRules = {
        ...
        regular_check_frequency: {
            required: function(element) {
                if ($('#category_id').val() != '' && $('#category_id').select2('data').text.toLowerCase() != "trailers") {
                    return true;
                }
                return false;
            }
...
$('.grid-clear-btn-workshop').on('click', function(event) {
    $('input[name="profiletitle"]').select2('val','');
    event.preventDefault();
    var grid = $("#jqGrid"), f;
    grid[0].p.search = false;
    f = {groupOp:"AND",rules:[]};
