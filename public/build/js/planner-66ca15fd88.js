    ...
    $(document).on("click",".js-event-detail",function() {
        ...
        setTimeout(function() {
            $.ajax({
                ...
                method: 'POST',
                data: {'key': key, 'selectedDate': selectedDate, 'type' : $('#event-category').val()},
                success: function(response) {
...
function getEventDetail(date) {
    ...
    $.ajax({
        ...
        method: 'POST',
        data:{
            selectedEvent : currentEvent, type : $('#event-category').val()
        },
        success: function(response) {
            $("#daily-events").html(response);
            ...
            console.log('currentEventCategory', currentEventCategory)
            $(".js-event-filter").val(currentEvent);
            $(".js-event-category").val(currentEventCategory);
...
function addNoEventMsg() {
    if(!$('.event-block:visible').length && !$('.js-no-event').length) {
        $('.planner-detail-card .js-planner-detail-card-data').append('<h4 class="col-md-12 block font-weight-700 js-no-event">There are no events scheduled for this date.</h4>');
    } else {
        $('.js-no-event').remove();
    }
}