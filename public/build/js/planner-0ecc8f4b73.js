...
var noVehicleAssetSelectedMsg = 'Vehicles or Assets must be selected';
...
var KeysAndColumnName = {
    ...
    lolerTest : {
        title : 'lolerTest',
        dateColumn : 'loler_test_date',
        countColumn : 'total',
        arrayKey : 'getLolerTest'
    },
    electricalInspection : {
        title : 'electricalInspection',
        dateColumn : 'electrical_inspection_date',
        countColumn : 'total',
        arrayKey : 'getElectricalInspection'
    },
    rubberIntegrityTest : {
        title : 'rubberIntegrityTest',
        dateColumn : 'rubber_integrity_test_date',
        countColumn : 'total',
        arrayKey : 'getRubberIntegrityTest'
    },
...
function getSelectedRegion() {
    return selectedRegion ? ($('.select2-region-calendar').val() == 'undefined' ? null : $('.select2-region-calendar').val()) : selectedRegion;
}
...
$(document).ready(function() {
   ...
   var date = moment(new Date()).format('MMM YYYY');
    ...
    $(document).on("click",".js-event-detail",function() {
        ...
        setTimeout(function() {
            $.ajax({
                ...
                method: 'POST',
                data: {'key': key, 'selectedDate': selectedDate, 'region' : getSelectedRegion()},
                success: function(response) {
                    $("#daily-events").html(response);
                    $("#processingModal").modal('hide');
                    var region = getSelectedRegion();
                    region = (region == '' || !region) ? 'all' : region;
                    $("#print-btn").attr("href", 'planner/exportSelectedEvents/' + selectedDate + '/' + slug + '/' + categoryType + '/' + region);
                    Metronic.init();
                }
            });
        }, 100);
    ...
    $('body').on('click','.current-month',function () {
        selectedEventDate = $(this).attr('data-date');
    ...
    $(document).on('click', '.js-event-types', function() {
        var r = $(this).val();
        checkEventType(r);
    });

    // $(document).on('click', '.js-event-category', function() {
    //     if($('.js-reset-filter-div').hasClass('d-none')) {
    //         $('.js-reset-filter-div').removeClass('d-none');
    //     }
    // });
    ...
    $(document).on('click', '#applyEventsFilter', function() {
        $(document).find('.js-event-category').each(function(k, v) {
            ...
            var r = $(this).data('slug')
            if($(this).is(':checked')) {
                var thisChecked = selectedEventFilter.includes(r);
        ...
        // $('.js-reset-filter-div').addClass('d-none');

        $('#closeEventsFilter').trigger('click');
    });

    $(document).on('click', '.js-remove-event', function() {
        var r = $(this).data('slug');
        if(r == 'assets' && $('.js-vehicles').length == 0) {
            toastr["error"](noVehicleAssetSelectedMsg);
            return false;
        } else if(r == 'vehicles' && $('.js-assets').length == 0) {
            toastr["error"](noVehicleAssetSelectedMsg);
            return false;
        }
        ...
        removeEventBadge(r);
        var $this = $(document).find('.js-event-category[data-slug='+ r +']');
        setTimeout(function() {
            $(document).find('.js-event-category[data-slug='+ r +']').parent().removeClass('checked');
            if($("#month_year_selector").val() == 'month') {
                var currentCalendar = $('#calendar').fullCalendar('getDate')._d;
                fullcalendarInit(currentCalendar);
            } else {
    ...
    $("#print-btn").on('click', function() {
        if($("#print-btn").attr('href') != 'javascript:void(0)') {
            window.location.href = $("#print-btn").attr('href');
            return false;
        }
        var url = 'planner/exportDayEvents/'+selectedEventDate;
        var data = {'selectedEvent' : getSelectedEvent(), 'region' : getSelectedRegion()};
    ...
    $(document).on('click', '#js-event-dropdown', function() {
        $('#event-filter-dropdown').toggle();
    });

    $(document).on('click', '#closeEventsFilter', function() {
        $(selectedEventFilter).each(function(k, r) {
            var $this = $(document).find('.js-event-category[data-slug='+ r +']');
        ...
        var difference = $(allEvents).not(selectedEventFilter).get();
        $(difference).each(function(k, r) {
            var $this = $(document).find('.js-event-category[data-slug='+ r +']');
            if($this.parent().hasClass('checked')) {
    ...
    $(document).on('click', '.js-reset-filter', function() {
        selectAllEvents('reset');
        setTimeout(function() {
            $(document).find('#applyEventsFilter').trigger('click');
        }, 100);
        // $('.js-reset-filter-div').addClass('d-none');
    });
...
function addEventBadge(r, eventName) {
    if(r == 'assets' || r == 'vehicles') {
        $('.js-add-event-badge').prepend('<div class="c-badge is-lg closeBtn mb-10 margin-right-10 js-'+r+'" style="white-space: nowrap;"><span class="small">'+eventName+'</span><button type="button" class="js-remove-event" aria-label="Close" data-slug="'+r+'"><svg stroke="currentColor" fill="none" viewBox="0 0 8 8"><path stroke-linecap="round" stroke-width="1.5" d="M1 1l6 6m0-6L1 7"></path></svg></button></div>');
    } else {
...
function checkEventType(r) {
    if(!$('.js-event-types[data-slug=vehicles]').is(':checked') && !$('.js-event-types[data-slug=assets]').is(':checked')) {
        $(document).find('.js-event-types[data-slug='+ r +']').parent().addClass('checked');
        $(document).find('.js-event-types[data-slug='+ r +']').trigger('click');
        toastr["error"](noVehicleAssetSelectedMsg);
        return false;
    }
    return true;
        ...
        success: function(response) {
            ...
            if(selectedEventFilter.length == 0) {
                ...
                $(selectedEventFilter).each(function(k, r) {
                    var $this = $(document).find('.js-event-category[data-slug='+ r +']');
                    var eventName = $this.data('event-name');
                    enableEventCategory(r, eventName)
                });
            }
...
function selectAllEvents(type = null) {
    selectedEventFilter = [];
    allEvents = [];
    $(document).find('.js-event-category').each(function(k, v) {
        var r = $(this).data('slug');
        var eventName = $(this).data('event-name');
        enableEventCategory(r, eventName, type)
        selectedEventFilter.push(r);
        allEvents.push(r);
    });
}

function enableEventCategory(r, eventName, type = null) {
    if(!type || type != 'reset') {
        addEventBadge(r, eventName);
    }
    setTimeout(function() {
        let event = $(document).find('.js-event-category[data-slug='+ r +']');
        if(!event.parent().hasClass('checked')) {
            $(document).find('.js-event-category[data-slug='+ r +']').parent().addClass('checked');
            $(document).find('.js-event-category[data-slug='+ r +']').trigger('click');
        }
    }, 50);
}
...
function addNoEventMsg() {
    setTimeout(function() {
        if(!$('.event-block:visible').length) {
            if(!$('.js-no-event').length) {
