...
function getUrlParameter(sParam) {
    ...
    if(sPageURL != '' && sParam == 'field' && sURLVariables.length > 0) {
        $('.js-vehicle-planning-tab').trigger('click');
        setTimeout(function() {
            $(document).find('select[name=asset_search_for]').val('').trigger('change');
        }, 100)
        $('#print-btn').hide();
    }
    ...
    if(typeof JSON.parse(vehiclesPlanningPrefsData.filters).rules[0] !== undefined){
        $.each( JSON.parse(vehiclesPlanningPrefsData.filters).rules, function(){
            ...
            if(this.field == 'tank_test_date'){
                $('select[name=search_for_date_range]').val($.cookie("search_for_date_range")).trigger('change');
                $('.select-time-period-group').show();
                $('#search_for').val("tank_test");
                $("#search_for").select2("val", "tank_test");
            }
        });
    }

    $('#vehiclePlanningJqGridPager_left .dropdownmenu').remove();
...
var gridOptions = {
    ...
    pager:"#vehiclePlanningJqGridPager",
    sortable: {
        update: function(event) {
            jqGridColumnManagment('vehiclePlanningJqGrid');
        },
        options: {
                    items: ">th:not(:has(#jqgh_vehiclePlanningJqGrid_details),:hidden)"
            }
    },
    onInitGrid: function () {
                ...
                if (rowObject.service_interval_type == 'Distance') {
                    ...
                    if (rowObject.next_service_inspection_distance != null) {
                        ...
                        if(rowObject.incomplete_distance != null) {
                            ...
                            var odometerReading = ' ' + (rowObject.odometer_setting == 'km' ? 'KM' : 'Miles');
                            if (difference <= 1000) {
                                return '<span class="label-text-danger bold">' + numberWithCommas(nextServiceValue) + odometerReading + nextServiceDate + '</span>';
                            } else if (difference > 1000 && difference <= 2000) {
                                return '<span class="label-text-warning bold">' + numberWithCommas(nextServiceValue) + odometerReading + nextServiceDate + '</span>';
                            } else if (difference > 2000 && difference <= 3000) {
                                return '<span class="label-text-success bold">' + numberWithCommas(nextServiceValue) + odometerReading + nextServiceDate + '</span>';
                            } else {
                                return '<span class="label-text">' + numberWithCommas(nextServiceValue) + odometerReading + nextServiceDate + '</span>';
                            }
                        }
                    } else {
                        return '';
...
if (typeof Site !== 'undefined' && typeof Site.sortname !== 'undefined') {
    ...
    if (Site.sortname === 'tank_test_date_original') {
...
// $('#vehiclePlanningJqGrid').jqGridHelper('addNavigation');
// changePaginationSelect();
// $('#vehiclePlanningJqGrid').jqGridHelper('addExportButton', {
//     fileProps: {"title":"vehicles_planning", "creator":"Lanes Group"},
//     url: '/vehicles/planning_data'+parameters
// });

changePaginationForVehiclePlanning();
$("#vehiclePlanningJqGrid").navGrid(
    "#vehiclePlanningJqGridPager", {
        excel: true,
        search: true,
        add: false,
        edit: false,
...
$("#vehiclePlanningJqGrid").navButtonAdd("#vehiclePlanningJqGridPager", {
    caption: "exporttest",
    id: "exportVehiclePlanningJqGrid",
    buttonicon: "glyphicon-floppy-save",
    onClickButton: function() {
        var options = {
            fileProps: { title: "vehicles_planning", creator: "Lanes Group" },
            url: "/vehicles/planning_data",
        ...
        var parameters = '';
        var period = '';
        if ($('#search_for').val() != '' && $('#search_for_date_range').val() != '' && $('#search_for').val() === 'preventative_maintenance_inspection') {
            period = getVehicleEventPeriodColor($('#search_for_date_range').val());
        ...
        // jQuery("#vehiclePlanningJqGrid").hideCol(['vehId', 'next_service_inspection_distance', 'vehicle_category', 'vehicle_type', 'service_interval_type', 'first_pmi_date_original', 'diff', 'last_pmi_inspection', 'incomplete_distance', 'incomplete_date', 'id', 'completed_pmi', 'next_distance_date', 'next_service_inspection_distance', 'vehicle_type', 'diff']);
...
$('#vehicles-planning-filter-form').on('submit', function(event) {
    ...
    period = getVehicleEventPeriodColor(search_for_date_range);
...
function clickVehicleExport() {
    $("#exportVehiclePlanningJqGrid").trigger("click");
}

function changePaginationForVehiclePlanning() {
    $pager = $("#vehiclePlanningJqGrid")
        .closest(".ui-jqgrid")
...
function getVehicleEventPeriodColor(search_for_date_range) {
    var period = '';
    if (search_for_date_range === 'Date passed') {
        period = 'other';
    }
