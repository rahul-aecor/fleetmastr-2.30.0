var assignmentDeleteRecordId = Site.assignmentDeleteRecordId;
    ...
    colModel: [
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                ...
                if(rowObject.slug == 'next_service_inspection_distance'){
                    return numberWithCommas(rowObject.event_planned_distance) + ' ' + (rowObject.odometer_setting == 'km' ? 'KM' : 'Miles');
                } else if(rowObject.event_plan_date == null && rowObject.slug == 'preventative_maintenance_inspection') {
                    return '';
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                if(rowObject.event_date == null){
                    ...
                    return moment(rowObject.event_date).format('DD MMM YYYY');
                }
            },
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                if(rowObject.odomerter_reading > 0){
                    return numberWithCommas(rowObject.odomerter_reading);
                }else {
                    return '';
                }
            }
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                ...
                if(rowObject.event_status == 'Complete') {
                    returnString+= '<a title="Delete" href="javascript:void(0);" class="btn btn-xs grey-gallery tras_btn delete_maintenance_history" data-maintenance-history-delete-id="'+ rowObject.id +'"><i class="jv-icon jv-dustbin icon-big"></i></a>';
                }
                return returnString;                
            }
        }
    ...
    height: "auto",
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                ...
                if(assignmentDeleteRecordId.includes(rowObject.id.toString())){
...
$(document).on('click', '.close_mot_schedule', function(){
    closeMotSchedule();
});
...
$(document).on('click','.js-update-pmi-schedule-edit-radio',function (event) {
    if($('.js-update-pmi-schedule-edit-radio:checked').val() == 0) {
        var pmiDate = $('.js-current-pmi-date-edit').html();
    } else {
        var pmiDate = $('.js-new-pmi-date-edit').html();
    }

    pmiDate = moment(pmiDate).format("DD MMM YYYY");
...
$(document).on('click','.roles-types-radio',function (event) {
    if($('.roles-types-radio:checked').val() == 0) {
        var motDate = $('#motAsPerOld').html();
    } else {
        var motDate = $('#motAsPerNew').html();
    }

    motDate = moment(motDate).format("DD MMM YYYY");
    $('#current_mot_date').val(moment(motDate).format("DD MMM YYYY"));
    $('.current_mot_calendar').datepicker('setDate', moment(motDate).format("DD MMM YYYY"));
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            setTimeout(function(){
                ...
                if($('#event_slug').val() == 'preventative_maintenance_inspection') {
                    let numWeeks = $('#event_interval').val().replace(' weeks', '');
...
$(document).on('click', '#editMaintenancePlannedDateLockBtn', function(){
    if($("#edit_maintenance_status").val()=="Incomplete"){
        let isAnyChange=false;
        if($("#edit_maintenance_planned_date").is(":disabled")==true){
            $("#edit_maintenance_planned_date").removeAttr("disabled");
            isAnyChange=true;
        }
        if($("#edit_maintenance_planned_date").is(":read-only")==true){
            $("#edit_maintenance_planned_date").removeAttr("read-only");
            isAnyChange=true;
        }
        if(isAnyChange==true){
            $("#edit_maintenance_planned_date").css({
                "background-color":"white",
                "cursor":"text"
            });
            $("#editMaintenancePlannedDateLockBtn > i").removeClass("jv-lock").addClass("jv-calendar");
        }
    }
});

$(document).on('click', '#editMaintenancePlannedDistanceLockBtn', function(){
    if($("#edit_maintenance_status").val() == "Incomplete"){
        $("#edit_maintenance_planned_distance").removeAttr("disabled");
        var eventPlannedDistance = $("#edit_maintenance_planned_distance").val().replace(/,/g, '');
        $("#edit_maintenance_planned_distance").val(eventPlannedDistance);
        $("#editMaintenancePlannedDistanceLockBtn > i").removeClass("jv-lock")
        $('#editMaintenancePlannedDistanceLockBtn').text($('#odometer_reading_unit_display').text());
    }
});
...
$(document).on('click', '#editMaintenanceHistorySave', function(e){
    ...
    var eventPlannedDistance = $('input[name="event_planned_distance"]').length ? $('input[name="event_planned_distance"]').val().replace(/,/g, '') : '';
    ...
    $.validator.addMethod("checkEventDateAndStatus", function(value, element) {
        return !(editVehicleMaintenanceStatus == 'Incomplete' && editVehicleMaintenanceEventDate != '');
    }, "Event date has been added. Status must be set to 'Complete'.");
    ...
    forumForm.validate({
        ...
        rules: {
            ...
            'edit_maintenance_planned_date': {
                required: {
                    depends: function(element) {
                        ...
                        return $('#edit_maintenance_planned_date').val() == '';
                    }
                },
            },
            ...
            'edit_maintenance_odometer_reading': {
                required: {
                    depends: function(element) {
                        ...
                        return ($("#edit_maintenance_status").val() === 'Complete' && $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'next_service_inspection_distance') ? true : false;
                    }
                },
                // digits: {
                //     depends: function(element) {
            ...
            'event_planned_distance': {
                required: {
                    depends: function(element) {
                        return $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'next_service_inspection_distance' ? true : false;
                    }
                },
                digits: true
            },
    ...
    if(Site.vehicle.first_pmi_date) {
        ...
        if($('#edit_maintenance_status').val() == 'Complete' && $('#current_event_status').val() != 'Complete' && $("#is_update_pmi_schedule_edit").val() == 'N/A' && $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'preventative_maintenance_inspection') {
            ...
            if(editVehicleMaintenanceEventDate != '' && dateToConsider != null && eventDate != dateToConsider) {
                ...
                $('#current_pmi_date').val('');
                $('.current_pmi_calendar').datepicker('setDate', '');
    ...
    if($('#edit_maintenance_status').val() == 'Complete' && $('#current_event_status').val() != 'Complete' && $("#is_update_mot_schedule_edit").val() == 'N/A' && $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'mot'){
        ...
        if($('input[name="updateMOT"]:checked').val()){
            ...
            if(eventDate != null && eventDate != '' && motAsPerEventDate != 'Invalid date' && moment(motAsPerPlanDate).format("DD MMM YYYY") != moment(motAsPerEventDate).format("DD MMM YYYY")){
                ...
                $('#current_mot_date').val('');
                $('.current_mot_calendar').datepicker('setDate', '');
    ...
    $.ajax({
        ...
        data: {
            ...
            'eventPlannedDistance': eventPlannedDistance,
    ...
    $.ajax({
        ...
        success:function(response){
            if(response.status == false) {
                ...
                $("#assignmentjqGrid").trigger("reloadGrid",[{page:1,current:true}]);
                toastr["success"]("Event updated successfully.");
                assignmentDeleteRecordId = response.assignmentDeleteRecordId;
            }
        },
    });
});
...
function initFormDate() {
    ...
    $(".maintenance_history_event_date").datepicker({
        format: "dd M yyyy",
        autoclose: true,
        clearBtn: true,
        todayHighlight: true,
