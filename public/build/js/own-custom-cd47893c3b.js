...
$(document).ready(function() {
    ...
    var this3;
    $('#updateAssetDocument').on('click','.doc-delete-btn1',function(){
       this3 = $(this);
               bootbox.confirm({
                title: "Confirmation",
                message: 'Are you sure you would like to delete this document?',
                callback: function(result) {
                    if(result) {
                       this3.closest(".delete-wrapper").find("button.delete").trigger("click");
                       return true;
                    }
                },
                buttons: {
...
function changePaginationSelectWithId(id){
...
function jqGridColumnManagment(grid = 'jqGrid')
{
    var jqGrid = $("#"+grid);
...
// for app dashboard image
$('#app_dashboard_image').on('change', function() {
    var file_name = $(this).val();
    if(file_name.length > 0) {
        var currImage = this;
        var reader = new FileReader();
        ...
        reader.onload = function (e) {
            ...
            image.src = e.target.result;
            image.onload = function () {
                if (this.height < 280 || this.width < 840) {
                    alert('Image must be at least 840px × 280px');
                } else {
                    cropForAppDashboardImg(image.src);
                }
            }
        }
    }
...
var cropForAppDashboardImg = function(image) {
    var imgNm = '.app-dashboard-image-prev';
    var imgContNm = '.app-dashboard-image-prev-container';

    $(imgNm).one("load", function() {
        $("#app_dashboard_image_upload").show();
        $("#app_dahboard_image_change_btn").text('Change image');
...
$("#app_dashboard_image_upload").click(function () {
    input = document.getElementById('app_dashboard_image');
    if (input.files && input.files[0]) {
        var formData = new FormData();
        formData.append('image', input.files[0]);
        ...
        var currentBtn = $(this);
        currentBtn.text('Uploading...').attr('disabled', 'disabled');
        $("#app_dahboard_image_change_btn").attr('disabled', 'disabled');

        $.ajax({
            url: '/settings/upload-app-dashboard-image',
            type: 'POST',
            data: formData,
            cache: false,
            processData: false,
        ...
        .done(function(data) {
            $('#app_dashboard_image_form_input').val(data);
            $('#app_dashboard_main_image').show().attr('src', data);
            currentBtn.text('Upload').hide().removeAttr('disabled');
            $("#app_dashboard_image").removeAttr('disabled');
            $("#app_dahboard_image_change_btn").removeAttr('disabled');
            $(".app-dashboard-image-prev-container").hide();
            $('.js-preview-container').addClass('d-none');
        })
        .fail(function(jqXHR, textStatus, errorThrown) {
            console.log('error', errorThrown);
        });
