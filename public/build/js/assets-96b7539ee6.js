   ...
   if ($().select2) {
    ...
    if($('select.asset-division-value').val() != '' && $('select.asset-division-value').val() != undefined) {
        ...
        if(Site.isRegionLinkedInVehicle) {
            ...
            $('#asset_region_id').empty();
            $('#asset_region_id').append('<option value></option>');
            $.each(Site.vehicleRegions[$('select.asset-division-value').val() ], function (key, val) {
                $('#asset_region_id').append('<option value="'+key+'">'+val+'</option>');
            });
    ...
    if($('select.asset-region').val() != ''){
        ...
        if(Site.isLocationLinkedInVehicle) {
            ...
            $('#vehicle_location_id').append('<option value></option>');
            if(typeof Site.vehicleBaseLocations[$('select.asset-region').val()] !== 'undefined') {
                $.each(Site.vehicleBaseLocations[$('select.asset-region').val()], function (key, val) {
                    $('#vehicle_location_id').append('<option value="'+key+'">'+val+'</option>');
                });
            }
