...
$(document).ready(function() {
    ...
    if(typeof JSON.parse(assetsPlanningPrefsData.filters).rules[0] !== undefined){
        $.each( JSON.parse(assetsPlanningPrefsData.filters).rules, function(){
            if(this.field == 'assets.vehicle_region_id'){
            ...
            if(this.field == 'dt_repair_expiry'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("maintenance_expiry");
                $("#asset_search_for").select2("val", "maintenance_expiry");
            }
            ...
            if(this.field == 'dt_tacograch_calibration_due'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("tachograph_calibration");
                $("#asset_search_for").select2("val", "tachograph_calibration");
            }
            if(this.field == 'dt_tax_expiry'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("vehicle_tax");
                $("#asset_search_for").select2("val", "vehicle_tax");
            }
            ...
            if(this.field == 'next_invertor_service_date'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("invertor_inspection");
                $("#asset_search_for").select2("val", "invertor_inspection");
            }
            if(this.field == 'next_pto_service_date'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("pto_service_inspection");
                $("#asset_search_for").select2("val", "pto_service_inspection");
            }
            if(this.field == 'next_compressor_service'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("compressor_inspection");
                $("#asset_search_for").select2("val", "compressor_inspection");
            }
            if(this.field == 'dt_loler_test_due'){
                $('select[name=asset_search_for_date_range]').val($.cookie("asset_search_for_date_range")).trigger('change');
                $('.asset-select-time-period-group').show();
                $('#asset_search_for').val("loler_test");
                $("#asset_search_for").select2("val", "loler_test");
            }
    ...
    colModel: [
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                return '<a title="" href="/assets/' + rowObject.vehId + '" class="font-blue">'+cellvalue+'</a>'
            }
        },
        {
            label: 'Division',
        ...
        },
        {
            ...
            formatter: function(cellvalue, options, rowObject) {
                ...
                var iconHtml = ' &nbsp;<a class="text-decoration-none" href="/assets/'+rowObject.vehId+'#maintenance_tab"><i id="pmi_interval_icon" class="js-pmi-inspection-duration js-maintenance-tab-pmi-filter fa fa-ban"></i></a>';
...
$('#vehicles-planning-filter-form').on('submit', function(event) {
    ...
    if (region && region != 'All') {
        f.rules.push({
            field: "assets.vehicle_region_id",
            op: "eq",
            data: region
        });
    }
