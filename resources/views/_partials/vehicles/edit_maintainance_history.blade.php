            ...
            <div class="col-md-12">
                <div class="form-group row gutters-tiny">
                    ...
                    <div class="col-md-9 error-class">
                        ...
                        <input type="hidden" name="event_slug" id="event_slug" value="{{ $vehicleMaintenancehistory->eventType->slug }}">
                        <input type="hidden" name="event_interval" id="event_interval" value="{{ $vehicleMaintenancehistory->eventType->slug == 'preventative_maintenance_inspection' ? $vehicleMaintenancehistory->vehicle->type->pmi_interval : '' }}">

                        <input type="hidden" name="is_update_pmi_schedule_edit" id="is_update_pmi_schedule_edit" value="N/A">

                        <input type="hidden" name="is_update_mot_schedule_edit" id="is_update_mot_schedule_edit" value="N/A">
                ...
                <div class="form-group row gutters-tiny">
                    {{-- <label class="col-md-3 control-label">Planned event<span class="js-planned-date">*</span>:</label> --}}
                    <label class="col-md-3 control-label">Planned event<span class="">*</span>:</label>
                    <?php
                        $disabledVehicleEvent = true;
                    ?>

                    @if($vehicleMaintenancehistory->eventType->slug == 'next_service_inspection_distance' && $vehicleMaintenancehistory->vehicle->type->service_interval_type == 'Distance')
                        <div class="col-md-9 error-class">
                            <div class="input-group {{ $vehicleMaintenancehistory->event_status == 'Complete' ? 'w-100' : '' }}">
                                <input type="text" size="16" disabled="disabled" class="form-control" name="event_planned_distance" id="edit_maintenance_planned_distance" value="{{ number_format($vehicleMaintenancehistory->event_planned_distance,0) }}">
                                @if($vehicleMaintenancehistory->event_status == 'Incomplete')
                                    <span class="input-group-btn">
                                        <button class="btn default date-set grey-gallery btn-h-45" type="button" id="editMaintenancePlannedDistanceLockBtn"><i class="jv-icon jv-lock"></i></button>
                                    </span>
                                @endif
                            </div>
                        </div>
                    @else
                        <div class="col-md-9 error-class">
                            <div class="input-group date {{ $vehicleMaintenancehistory->event_status != 'Complete'  ? 'maintenance_history_form_date' : 'w-100' }}">
                                <input type="text" size="16" {{ $disabledVehicleEvent ? 'readonly' : '' }} {{ $disabledVehicleEvent ? 'disabled="disabled"' : '' }} class="form-control" name="edit_maintenance_planned_date" id="edit_maintenance_planned_date" value="{{isset($vehicleMaintenancehistory->event_plan_date) ? $vehicleMaintenancehistory->event_plan_date : ''}}" placeholder="Select">
                                @if($vehicleMaintenancehistory->event_status != 'Complete')
                                    <span class="input-group-btn">
                                        <button class="btn default date-set grey-gallery btn-h-45" {{ $vehicleMaintenancehistory->event_status == 'Complete' ? 'disabled="disabled"' : '' }} type="button" id="editMaintenancePlannedDateLockBtn"><i class="jv-icon jv-lock"></i></button>
                                    </span>
                                @endif
                            </div>
                        </div>
                ...
                <div class="form-group row gutters-tiny">
                    ...
                    <div class="col-md-9 error-class">
                        <div class="input-group {{ $vehicleMaintenancehistory->event_status != 'Complete'  ? 'date form_date maintenance_history_event_date' : 'w-100' }}">
                            <input type="text" size="16" class="form-control bg-white cursor-pointer" name="edit_maintenance_event_date" id="edit_maintenance_event_date" value="{{isset($vehicleMaintenancehistory->event_date) ? $vehicleMaintenancehistory->event_date : ''}}" @if( $vehicleMaintenancehistory->event_status == 'Complete' ) disabled style="background: #f4f4f4 !important;cursor: not-allowed !important;" @else readonly @endif placeholder="Select">
                            @if($vehicleMaintenancehistory->event_status != 'Complete')
                                <span class="input-group-btn">
                ...
                <div class="form-group row gutters-tiny js-maintenance-odometer-reading">
                    <label class="col-md-3 control-label">Odometer reading {!! $vehicleMaintenancehistory->eventType->slug == 'next_service_inspection_distance' ? '<span class="js-required">*</span>' : '' !!}:</label>
                    <div class="col-md-9 error-class">
                        <div class="input-group">

                            {{-- @if($vehicleMaintenancehistory->eventType->slug == 'next_service_inspection_distance')

                                <input type="text" size="16" class="form-control bg-white cursor-pointer" name="edit_maintenance_odometer_reading" id="edit_maintenance_odometer_reading" value="{{isset($vehicleMaintenancehistory->event_planned_distance) ? $vehicleMaintenancehistory->event_planned_distance : ''}}">
                ...
                <div class="form-group row gutters-tiny">
                    ...
                    <div class="col-md-9 error-class">
                        <input type="hidden" id="current_event_status" value="{{ $vehicleMaintenancehistory->event_status }}">
                        <select class="form-control select2me select2-edit-maintenance-status" placeholder="Select" id="edit_maintenance_status" name="edit_maintenance_status" {{ $vehicleMaintenancehistory->event_status == 'Complete' ? 'disabled' : '' }}>
