...
$(document).ready(function() {
    ...
    if ($().select2) {
        ...
        $('.select2-asset-check-type').select2({
...
$('#assets_number, #asset_checks_created_by').on('change', function() {
    $('#asset_checks_quick_filter_form').trigger('submit');
});

$('#asset_checks_region, #assetCheckType, #asset_checks_status').on('change', function() {
    $('#asset_checks_advanced_filter_form').trigger('submit');
});
...
$('#asset_checks_advanced_filter_form').on('submit', function(event) {
    ...
    var assetCheckType = $('select[name="assetCheckType"]').val();
