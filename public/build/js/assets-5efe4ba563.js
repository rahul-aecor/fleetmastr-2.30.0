   ...
   if ($().select2) {
    ...
    if (typeof Site !== 'undefined') {
       var globalset = Site.column_management;
        ...
        if (Site != undefined && typeof Site.assetProfileTypeId !== 'undefined' && Site.assetProfileTypeId!=null && Site.assetProfileTypeId>0) {
            $("#type").select2("val", Site.assetProfileTypeId);
            $("#asset_advanced_search").trigger('click');
            let assetsPrefsData = JSON.stringify({"groupOp":"AND","rules":[{"field":"assets.deleted_at","op":"eq","data":null},{"field":"asset_profiles.id","op":"eq","data":Site.assetProfileTypeId}]});
            gridOptions = $.extend(gridOptions, {postData: {"showActiveAssetsOnly":true, "filters": assetsPrefsData}});
        }else{
    ...
    document.addEventListener('keydown', assetSelectKeydownHandler, true);
...
function assetSelectKeydownHandler(event){
    if(event.srcElement.id == 's2id_autogen2_search') {
        setTimeout(function() {
            $('.quick-filter-form-submit').trigger('click');
        }, 20);
    }
...
$('.quick-filter-form-submit').on('click', function(event) {
    ...
    if(serial_number == '' || !serial_number) {
        serial_number = $('#s2id_autogen2_search').val();
    }
...
function setSerialNumbersDropdown()
{
    if($("#show_archived_assets").length) {
        ...
        $.each(JSON.parse(data), function(key,value) {
            $el.append($("<option></option>")
                .attr("value", value.serial_number).text(value.serial_number));
        });
