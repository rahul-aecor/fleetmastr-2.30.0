 ...
 //        ]
	...
	$('#jqGrid1').jqGridHelper({
        ...
        colModel:[
        	...
        	// },
        	{
                label: '<div class="checker bulk-record"><span><input type="checkbox" onClick="bulkSelectAll($(this))" style="margin-right:5px;"></span></div>',
                name: 'id',
                width: 80,
                hidden: false,
                align: "center",
                ...
                hidedlg: true,
                showongrid: true,
                classes: "bulk-upload-data report-id",
                formatter: function(cellvalue, options, rowObject) {
                    if(rowObject.status == 'Accepted'){
                        return '<div class="checker disabled"><span><input type="checkbox" disabled value="" style="margin-right:5px;"></span></div>';
                    } else {
                        localStorageData = [];
...
$(document).on('click',".bulk-upload-data", function(event) {
    ...
    if ($($(this).find('span')).hasClass('checked')){
        ...
        $('#bulkDeleteRecords').removeAttr('disabled');
    }
    ...
    if(localStorageData.length == 0) {
        $('#bulkDeleteRecords').attr('disabled','disabled');
    }
    bulkCheckBoxCount();
});


function bulkSelectAll(e) {
    $('#bulkDeleteRecords').removeAttr('disabled');
...
$( "#bulkDeleteButton" ).on('click', function(event) {
        ...
        $.ajax({
            url: '/reports/bulk_delete',
            dataType: 'html',
            type: 'POST',
            data: { report_ids: localStorageData },
            success:function(response) {
                $('.bulk-record').click().find('span').removeClass('checked');
                $('#bulkDelete').modal('hide');
                $("#bulkDeleteRecords").html('Bulk deletion');
                var grid = $("#jqGrid1"), f;
                grid.trigger("reloadGrid",[{page:1,current:true}]);
                if (response == 'true'){
                    $('#bulkDeleteRecords').attr('disabled','disabled');
                    toastr['success']('Bulk reports are deleted successfully', 'Success');
                    localStorage.removeItem('localStorageData');
                }
            },
...
function bulkCheckBoxCount() {
    ...
    if(bulkAssigncount) {
        $("#bulkAssignedRecord").html('Bulk assigning ('+bulkAssigncount+')');
        $("#bulkDeleteRecords").html('Bulk deletion ('+bulkAssigncount+')');
    } else {
        $("#bulkAssignedRecord").html('Bulk assigning');
        $("#bulkDeleteRecords").html('Bulk deletion');
    }
}

function disableMainBulkActionCheckbox() {
    var isDisabled = true;
    $('.bulk-upload-data [type=checkbox]').each(function(){
        if (!$(this).closest('div.checker').hasClass('disabled')) {
            isDisabled = false;
        }
    });
    if (isDisabled) {
        $('.js-job-bulk-assign').addClass('disabled').find('span input:checkbox').attr('disabled', 'disabled');
    }
}
