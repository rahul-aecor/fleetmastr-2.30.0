$(document).ready(function() {
    ...
    $('#date_range').on('apply.daterangepicker', function(ev, picker) {
    	var startDate = moment(picker.startDate);
  		...
  		console.log('end-st', endDate.diff(startDate, 'days'))
  		if (endDate.diff(startDate, 'days') > 30) {
	    	$('input[name="date_range"]').val('');
	  		toastr["error"]('Please select dates range between 31 days');
	  		picker.show();    
	    } else {
	...
	$('#btnSaveCustomReport').on('click', function(e) {
	    ...
	    $(formId).validate({
            ...
            rules: {
		        ...
		        'date_range': {
		            required: function(element) {
						return Site.reportSlug != 'standard_last_login_report';
		            }
		        },
		        'field_name[]': {
            ...
            submitHandler: function (form) {
                ...
                $("#reportSummary tbody").sortable('disable');
	...
	$(document).on('click', '#customiseData', function() {
		...
		$("#reportSummary tbody").sortable('enable');
...
function handleSorting()
{
	...
	if(Site.reportSlug != 'standard_fleet_cost_report') {
