  $(document).on('click', '#view_edit_company', function(e){
    var redirect = $('#view_edit_company').data('path');
    var edit_tbody_id = "view_all_edit_companies";
    viewAllCompanies(redirect, edit_tbody_id);
  });
            ...
            var len = newOptions.length;
              for(var i=0; i<len; i++){
                  ...
                  if (user_company.length > 0 || defect_history_company.length > 0) {
                    ...
                    delete_url = "<a href='#' data-redirect=" + redirect + " data-id=" + id + " class='btn btn-xs grey-gallery edit-timesheet tras_btn js-company-edit-delete-btn' title='Delete the company' data-confirm-msg='Are you sure you want to delete this company?'><i class='jv-icon jv-dustbin icon-big'></i></a>";
                  }
  ...
  $(document).on('click', ".js-company-edit-delete-btn", function(){
      var id = $(this).data('id');
      ...
      bootbox.confirm({
              ...
              if(result) {
                $.ajax({
                  ...
                  success: function(response){
                    ...
                    viewAllCompanies(redirect, "view_all_edit_companies");
          ...
          buttons: {
              cancel: {
                  className: "btn white-btn btn-padding white-btn-border col-md-6 pull-left edit-company-button",
                  label: "Cancel"
              },
              confirm: {
  ...
  function select2DropDown(redirect, response) {
    ...
    var $el = $("#company_id");
    $el.empty(); // r
    var $el_edit = $("#company_id1");
    $el_edit.empty(); // remove old options
    if (redirect == 'user') {
      $.each(response, function(key,value) {
        $el_edit.append($("<option></option>")
  ...
  function duplicateCompanyNameExists(cname, companyId){
      var IsExists = false;
      $('#company_id1 option').each(function(){
