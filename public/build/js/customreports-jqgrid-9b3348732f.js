...
function searchReportDownloads() {
    ...
    grid[0].p.search = true;
    grid[0].p.postData = {filters:JSON.stringify(f)};
    grid.jqGrid('setGridParam', { url: '/customreports/download_data'}).trigger("reloadGrid",[{page:1,current:true,modalshow:false}]);
}
...
function reportDataDisplayModal(id, slug = null) {
	...
	$.ajax({
        ...
        success:function(response) {
			...
			if(slug != 'standard_last_login_report') {
				var idMonthArray = ['standard_defect_report', 'standard_fleet_cost_report', 'standard_driving_events_report', 'standard_speeding_report', 'standard_journey_report', 'standard_fuel_usage_and_emission_report', 'standard_driver_behavior_report', 'standard_vehicle_behavior_report', 'standard_asset_cost_report'];
			...
			if(slug == 'standard_last_login_report' || slug == 'standard_driver_behavior_report') {
