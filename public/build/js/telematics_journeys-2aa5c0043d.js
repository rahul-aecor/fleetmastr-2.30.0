...
function getJourneyFilterData() {
    ...
    var anomalyStatusFilterValue = $("#anomalyStatusJourney").val();
...
function clearJourneyFilter() {
    ...
    $("#anomalyStatusJourney").val('').change();
    getJourneyTabData();
}
...
jQuery("#journeyJqGrid").jqGrid({
    ...
    colModel: [{
        ...
        },
        {
            ...
            title: false,
            formatter: function(cellvalue, options, rowObject) {
                if(!rowObject.tj_isAnomaly){
                   var appendAnomalyStatusButtonIcon='<i class="fa fa-ban icon-big" style="color:red;"></i>';
                }else if(rowObject.tj_isAnomaly==1){
                   var appendAnomalyStatusButtonIcon='<i class="fa fa-check-circle icon-big" style="color:green;"></i>';
                }
                return '<button class="btn btn-xs grey-gallery tras_btn anomalyStatusBtn saveJourneyAnomaly" data-related-journey-id="'+rowObject.id+'">'+appendAnomalyStatusButtonIcon+'</button>';
            }
        }
    ],
