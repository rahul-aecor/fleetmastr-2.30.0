...
var isEventDetailsShowing = false;
...
$(document).ready(function() {
   ...
   var date = moment(new Date()).format('MMM YYYY');
    ...
    $(document).on("click",".js-event-detail",function() {
        ...
        var categoryType = $('#event-category').val();
        ...
        setTimeout(function() {
            $.ajax({
                ...
                method: 'POST',
                data: {'key': key, 'selectedDate': selectedDate, 'type' : getSelectedEventCategory(), 'region' : $('.select2-region-calendar').val()},
                success: function(response) {
                    $("#daily-events").html(response);
                    $("#processingModal").modal('hide');
                    isEventDetailsShowing = true;
                    // $("#print-btn").attr("href", 'planner/exportSelectedEvents/' + date + '/' + key + '/' + categoryType);
                }
            });
        }, 100);
    });
    ...
    $('body').on('change', '.js-event-filter', function () {
        ...
        currentEventCategory = $('#event-category').val();
        $(".event-block").hide();
        var hrefstring = $("#print-btn").attr("href");
        var hrefparts = hrefstring.split('/');
        var date = hrefparts[2];
        ...
        if(currentVal == '' || currentVal.toLowerCase() == 'all') {
            $("#print-btn").attr("href", 'planner/exportDayEvents/' + date + '/all/'+$('#event-category').val());
            $('#calendar').fullCalendar('removeEvents');
            $('#calendar').fullCalendar('refetchEvents');
            $('.event-block').show();
        } else {
            $("#print-btn").attr("href", 'planner/exportDayEvents/' + date + '/'+currentVal+'/'+$('#event-category').val());
            $("."+currentVal).show();
            if($('#event-category').val() == 'all' || $('#event-category').val() == 'vehicles') {
                $("."+currentVal+'_vehicle').show();
            }
            if($('#event-category').val() == 'all' || $('#event-category').val() == 'assets') {
                $("."+currentVal+'_asset').show();
            }
        }
    ...
    $('body').on('change', '#event-category', function () {
        var currentVal = $('#event').val();
        currentEvent = currentVal;
        currentEventCategory = $(this).val();
        $(".event-block").hide();
        var hrefstring = $("#print-btn").attr("href");
        var hrefparts = hrefstring.split('/');
        var date = hrefparts[2];
    ...
    $("#print-btn").on('click', function() {
        if(isEventDetailsShowing) {
            var categoryType = $('#event-category').val();
            var key = $(this).data('key');
            var selectedDate = $(this).data('date');
            var url = 'planner/exportSelectedEvents/';
        } else {
...
function getEventDetail(date) {
    isEventDetailsShowing = false;
    currentCalendar = date;
    // var data = {'selectedEvent' : currentEvent, 'type' : getSelectedEventCategory(), 'region' : $('.select2-region-calendar').val()};
    var data = {'selectedEvent' : getSelectedEvent(), 'region' : $('.select2-region-calendar').val()};
    $("#processingModal").modal('show');
    $.ajax({
        ...
        success: function(response) {
            ...
            // $(".js-event-category").select2('val', currentEventCategory);
            ...
            console.log('selectedEventFilter.length', selectedEventFilter.length)
