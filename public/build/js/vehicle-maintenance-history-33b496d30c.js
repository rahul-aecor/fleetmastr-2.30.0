    ...
    forumForm.validate({
        ...
        rules: {
            ...
            'maintenance_mot_type' : {
                required: {
                    depends: function(element) {
                        return  $("#maintenance_event_type").select2().find(":selected").data("slug") == 'mot' ? true : false;
                    }
                },
            },
            ...
            'maintenance_odometer_reading': {
                ...
                digits: {
                    depends: function(element) {
                        return $("#maintenance_event_type").select2().find(":selected").data("slug") == 'next_service_inspection_distance' ? true : false;
                    }
                }
            }
        },
...
function firstPmiDateWeekCalculation(type){
    ...
    console.log('firstPMIDate', firstPMIDate)
    console.log('nextPMIDate', nextPMIDate)
    console.log('firstPmiDateWeeks', firstPmiDateWeeks)
    ...
    if(type == 'edit') {    
        ...
        $('.js-current-pmi-date-edit').html(pmiDate.format("DD MMM YYYY"));
    } else {
        $('.js-new-pmi-date').html(newPMIDate);
        $('.js-current-pmi-date').html(pmiDate.format("DD MMM YYYY"));
    }
...
$(document).on('click','#maitenancePMIupdate',function (event) {
    ...
    if($("#frmMaitenancePMIupdate").valid()){
        alert('add: '+$('.js-update-pmi-schedule-radio:checked').val())
    ...
    forumForm.validate({
        ...
        rules: {
            ...
            'edit_maintenance_mot_type' : {
                required: {
                    depends: function(element) {
                        return $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'mot' ? true : false;
                    }
                },
            },
            ...
            'edit_maintenance_odometer_reading': {
                ...
                digits: {
                    depends: function(element) {
                        return ($("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'next_service_inspection_distance' && $("#edit_maintenance_status").val() === 'Complete') ? true : false;
                    }
                }
            }
        },
...
$(document).on('change','#edit_maintenance_status', function (event) {
    if($("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'next_service_inspection_distance' || $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'preventative_maintenance_inspection') {
        ...
        if($("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'preventative_maintenance_inspection') {
            $('.js-required.js-planned-date').show();
        }
    }
});
