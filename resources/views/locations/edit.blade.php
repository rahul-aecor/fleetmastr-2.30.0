...
<div class="page-bar">
    {!! Breadcrumbs::render('location_details_edit', $location['id']) !!}
</div>
<div class="row">
    {!! BootForm::openHorizontal(['md' => [3, 6]])->addClass('form-bordered form-validation location-form')->action('/locations/'.$location['id'])->id('editLocation')->put() !!}
                            ...
                            <div class="flex-grow-1 margin-right-15" style="width: calc(100% - 109px);">
                                <select class="form-control select2me " id="category_id" name="category_id">
                                    ...
                                    @foreach($locationCategories as $locationCategory)
                                        <option value="{{ $locationCategory['id'] }}" {{ $locationCategory['id'] == $location['location_category_id'] ? 'selected' : '' }}>{{ $locationCategory['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div style="flex-shrink: 0">
                                <div class="d-flex">
                                    <a href="#add-category" data-toggle="modal" class="align-items-center btn btn-blue-color btn-h-45 d-flex justify-content-center">
                                        <i class="jv-icon jv-plus"></i>
                                    </a>
                                    <a href="#view-category" id="view_category" data-path="user" data-toggle="modal" class="align-items-center btn btn-blue-color btn-h-45 d-flex justify-content-center" style="margin-left: 0">
                    ...
                    <div class="col-md-6">
                        <div class="row gutters-tiny">
                            <div class="col-md-10">
                                ...
                                <input type="hidden" class="form-control" name="latitude" id="latitude" value="{{ $location->latitude }}">
                                <input type="hidden" class="form-control" name="longitude" id="longitude" value="{{ $location->longitude }}">
                            </div>
                            <div class="col-md-2">
                                <button type="button" class="btn-block align-items-center btn btn-blue-color btn-h-45 d-flex justify-content-center js-find-pincode-btn">Find</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="row">
    <div class="col-md-6 col-md-offset-3 btn-group">
        ...
        <button type="submit" class="btn red-rubine btn-padding col-md-6 js-edit-location" id="submit-button">Save</button>
   </div>
</div>
{!! BootForm::close() !!}
