...
$(document).ready(function() {
    ...
    document.addEventListener('keydown', registrationSelectKeydownHandler, true);
...
function registrationSelectKeydownHandler(event){
    if(event.srcElement.id == 's2id_autogen2_search') {
        setTimeout(function() {
            let val = $('#'+event.srcElement.id).val();
            if((val.length > 2 && val.length < 8) || val.length == 0) {
                $('#vehicles-quick-filter-form').trigger('submit');
            }
        }, 20);
    }
...
if (typeof Site !== 'undefined' && typeof Site.vehicleType !== 'undefined') {
    $( "#type" ).select2('val', Site.vehicleType);
    gridOptions = $.extend(gridOptions, {postData: {'showActivevehiclesOnly':true, 'filters': JSON.stringify($.extend(JSON.parse(vehiclesPrefsData.filters),Site.filters))}});
}
