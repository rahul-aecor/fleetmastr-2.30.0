        	...
        	date_range: {
	            required: {
	                depends: function(element) {
	                    // return $('#reportId').val() != 6 ? true : false;
...
function reportDataDisplayModal(id, slug = null) {
	// if(slug && slug == 'standard_last_login_report') {
	// 	$('.js-division-container').hide();
	// 	$('#all_accessible_region').trigger('click');
	// } else {
	// 	$('.js-division-container').show();
	// }
	...
	$('#download_report_modal input[type="checkbox"]').prop('checked', false).uniform('refresh');
	$.ajax({
        ...
        success:function(response) {
        	...
        	if(slug != 'standard_last_login_report') {
        		// var idMonthArray = [1, 5];
	        	// var idWeekArray = [2, 3, 4];
        		var idMonthArray = ['standard_defect_report', 'standard_fleet_cost_report'];
	        	var idWeekArray = ['standard_vor_defect_report', 'standard_vor_report', 'standard_activity_report'];
	        	var dateRange;
	        	var start;
	    		var end;
...
function showReportCalender(dateRange, start, end, slug) {
	...
	$('input[name="date_range"]').on('show.daterangepicker', function(ev, picker) {
		if(slug) {
	  		$('.daterangepicker .ranges ul li:last').addClass('d-none');
	  	} else {
	  		$('.daterangepicker .ranges ul li:last').removeClass('d-none');
	  	}
	});
}
...
function downloadDataDisplayModal(id) {
	$.ajax({
        ...
        success:function(response) {
			...
			console.log('isCustomReportFlag', isCustomReportFlag);
			console.log('isAutoDownloadFlag', isAutoDownloadFlag);
	...
	$('#reportsJqGrid').jqGridHelper({
        ...
        colModel:[
            ...
            },            
            {
	        	...
	        	align: 'left',
	        	formatter: function( cellvalue, options, rowObject ) {
	        		return rowObject.last_generated_date ? moment(rowObject.last_generated_date).format('hh:mm:ss DD MMM YYYY')+ ' (<a title="Details" href="'+ rowObject.filename +'" download" class="underline text-primary js-show-data report_download">Download</a>)' : '';
                }
	        },
            {
                label: 'Actions',
                ...
                formatter: function( cellvalue, options, rowObject ) {
                	...
                	return '<a title="Details" href="javascript:void(0)" onClick="reportDataDisplayModal(' + rowObject.id + ', ' + slug + ')" class="btn btn-xs grey-gallery tras_btn"><i class="jv-icon jv-find-doc js-show-data text-decoration icon-big"></i></a>';
                }
            }
        ]
	});
	...
	$('#jqGrid1').jqGridHelper({
        ...
        colModel:[
	        ...
	        },
	        {
                ...
                width: 150,
                formatter: function( cellvalue, options, rowObject ) {
                    return rowObject.is_custom_report == 0 ? 'Standard' : 'Custom';;
                }
            },
	        {
	        	label: 'Category',
