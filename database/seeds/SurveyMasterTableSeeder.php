...
class SurveyMasterTableSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        if($brandName === 'ferns') {
            $jsonData['title'] = 'Tanker and Trailer Check';
            $jsonData['text'] = 'Is there a tanker or trailer attached to the vehicle?';
        } else {
            $jsonData['title'] = 'Trailer Check';
            $jsonData['text'] = 'Is there a trailer attached to the vehicle?';
        }
        ...
        foreach ($surveys as $survey) {
            ...
            $trailerQuestionsOrder = env('TRAILER_QUESTIONS_ORDER') != '' ? explode(',', env('TRAILER_QUESTIONS_ORDER')) : [];
            ...
            $newSurvey->screen_json = $this->getCheckoutJson($survey->vehicle_category, $survey->defect_list, $skipDefects, $removeDefects, $AdBlueRequired, $trailerQuestionsOrder, $jsonData);
            $newSurvey->save();
            ...
            $newSurvey->screen_json = $this->getCheckoutOnCallJson($survey->vehicle_category, $survey->defect_list, $skipDefects, $removeDefects, $AdBlueRequired, $trailerQuestionsOrder, $jsonData);
            $newSurvey->save();
            ...
            $newSurvey->screen_json = $this->getCheckinJson($survey->vehicle_category, $survey->defect_list, $skipDefects, $removeDefects, $AdBlueRequired, $trailerQuestionsOrder, $jsonData);
            $newSurvey->save();
            ...
            $newSurvey->screen_json = $this->getDefectJson($survey->vehicle_category, $survey->defect_list, $skipDefects, $removeDefects, $AdBlueRequired, $trailerQuestionsOrder, $jsonData);
            $newSurvey->save();
    ...
    private function getCheckoutJson($vehicleCategory, $orderList, $orderListSkip, $orderListRemove, $AdBlueRequired, $trailerQuestionsOrder, $jsonData)
    {
        $screen_no = 1;
        $action_json1 = "";
        $multiselect_json = "";
        ...
        if(!empty($trailerQuestionsOrder)) {
            $yesno_query = $yesno_query->whereNotIn('order', $trailerQuestionsOrder);
        }
        ...
        foreach ($yesno_query_data as $row) {
            ...
            } else if($row->type == 'declaration') {
                $yesno_json .= '
                    {
                        "_number": "' . $screen_no . '",
                        "screen_title": "Declaration",
                        "_type": "declaration",';
            } else {
                $yesno_json .= '
                    {
            ...
            $onContinueBtn = $row->type == 'declaration' ? ($screen_no + 1) : "";
            $yesno_json .= '"buttons": {
                        "show_save": "yes",
                        "show_continue": "yes"
                    },
        ...
        if(setting('is_trailer_feature_enabled')) {
            $trailerQuestionsQuery = \DB::table('defect_master')
                ...
                ->whereIn('order', $trailerQuestionsOrder)
                ->groupBy('order')
                ->get();
    ...
    private function getCheckoutOnCallJson($vehicleCategory, $orderList, $orderListSkip, $orderListRemove, $AdBlueRequired, $trailerQuestionsOrder, $jsonData)
    {
        $screen_no = 1;
        $action_json1 = "";
        $multiselect_json = "";
        ...
        if(!empty($trailerQuestionsOrder)) {
            $list_query = $list_query->whereNotIn('order', $trailerQuestionsOrder);
        }
    ...
    private function getCheckinJson($vehicleCategory, $orderList, $orderListSkip, $orderListRemove, $AdBlueRequired, $trailerQuestionsOrder, $jsonData)
    {
        $screen_no = 1;
        $action_json1 = "";
        $multiselect_json = "";
        ...
        $list_query = $list_query->where('order','>=',0);
        $list_query = $list_query->where('type','!=','declaration');
        if(!empty($trailerQuestionsOrder)) {
            $list_query = $list_query->whereNotIn('order', $trailerQuestionsOrder);
        }
        $list_query =  $list_query->whereIn('order', explode(',', $orderList))
    ...
    private function getDefectJson($vehicleCategory, $orderList, $orderListSkip, $orderListRemove, $AdBlueRequired, $trailerQuestionsOrder, $jsonData)
    {
        $screen_no = 1;
        $trailerAttachedJson = "";
        $dynamicDefects = json_decode(env('HAVING_DYNAMIC_DEFECTS'),true);
