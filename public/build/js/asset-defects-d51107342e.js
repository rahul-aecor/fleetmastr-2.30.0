...
$('#defectStatusSave').on('click', function() {
    if($("#comment").val() ==''){
        ...
        if(defectStatusEditableValue == 'Resolved' && Site.assetDefectStatusCount == 1 && Site.assetDefectStatus != "Roadworthy") {
...
$('#defects-quick-filter-form').on('submit', function(event) {
    ...
    if((defectID != '' && assetNumber!= '') || (defectID != '' && driver_id!='') || (assetNumber != '' && driver_id!='')) {
...
$(document).ready(function() {    
    ...
    $(document).on('click','#defect_status_resolved_yes', function() {
        $.ajax({
            url: '/assets/defect/updateDefectStatus',
            type: 'POST',
            cache: false,
            data: { 'assetId': Site.defectAssetId },
            success:function(response){
               toastr["success"]("Data updated successfully.");
               $('#defect_status_resolved').modal('hide');
               $('.asset-status-edit').editable('setValue', response);
               $('.asset-status-view').text(response);
            }
        });
    });
...
$('#serial_number, #driver_id').on('change', function() {
