...
$(document).ready(function() {
   ...
   if ($().select2) {
    ...
    $("#addAssetAllocationSave").on('click',function(){
        ...
        var nameVal = $('input[name="asset_allocation_name"]').val();
        $('input[name="asset_allocation_name"]').val($.trim(nameVal));
        var $button = $(this);
        //setTimeout(function () {
            forumForm.validate({
                ignore: [],
            ...
            if(duplicateAssetAllocationName($.trim(nameVal), '')) {
                $( "#name-error" ).parent().removeClass( "has-error" );
                $( "#name-error" ).remove();
                validFlag = false;
    ...
    if(Site.isAllocation != 'undefined' && Site.isAllocation == 1 && typeof Site.isSuperAdmin != 'undefined' && Site.isSuperAdmin && typeof Site.page != 'undefined' && Site.page == 'edit') {
        $('.allocation_auth').removeClass("hide");
        $('.allocation_div').addClass("hide");
    } else {
        $('.allocation_auth').addClass("hide");
...
$('#asset_profile_id').on('change', function(event) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(response.is_allocation == 1 && typeof Site.isSuperAdmin != 'undefined' && Site.isSuperAdmin) {
                $('.allocation_auth').removeClass("hide");
                $('.allocation_div').addClass("hide");
            } else {
                $('.allocation_auth').addClass("hide");
