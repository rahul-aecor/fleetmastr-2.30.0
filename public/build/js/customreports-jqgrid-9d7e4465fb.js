		...
		var startDate = moment(picker.startDate);
  		...
  		if (endDate.diff(startDate, 'days') > 30) {
	    	$('input[name="date_range"]').val('');
	  		toastr["error"]('Please select date range between 31 days');
	  		picker.show();    
	    }
        	...
        	date_range: {
	            required: {
	                depends: function(element) {
						return $('#reportSlug').val() != 'standard_last_login_report' && $('#reportSlug').val() != 'standard_user_details_report' ? true : false;
	                }
	            }
	        },
            'accessible_regions[]': {
...
function reportDataDisplayModal(id, slug = null) {
	if(slug == 'standard_last_login_report' || slug == 'standard_user_details_report') {
        ...
        colModel:[
	        ...
	        // },
	        {
                ...
                align: 'left',
  		        width: 250,
                formatter: function( cellvalue, options, rowObject ) {
					if(rowObject.slug == 'standard_last_login_report' || rowObject.slug == 'standard_user_details_report') {
