...
function firstPmiDateWeekCalculation(type){
    ...
    if(type == 'edit') {
        ...
        // $('#current_pmi_date').datepicker({ dateFormat: 'DD MMM YYYY' }); // format to show
        // $('#current_pmi_date').datepicker('setDate', pmiDate);
        $('#current_pmi_date').val(pmiDate);
        $('#current_pmi_date').attr('disabled', 'disabled');
        $('.current_mot_calendar').datepicker('setDate', pmiDate);
    } else {
...
$(document).on('click', '#editMaintenanceHistorySave', function(e){
    ...
    if(Site.vehicle.first_pmi_date) {
        ...
        if($('#edit_maintenance_status').val() == 'Complete' && $("#is_update_pmi_schedule_edit").val() == 'N/A' && $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'preventative_maintenance_inspection') {
            var firstPMIDate = Date.parse(Site.vehicle.first_pmi_date);
            var nextPMIDate = Date.parse(Site.vehicle.next_pmi_date);
            var currentDate = Date.parse(new Date());
    ...
    if($('#edit_maintenance_status').val() == 'Complete' && $("#is_update_mot_schedule_edit").val() == 'N/A' && $("#edit_maintenance_event_type").select2().find(":selected").data("slug") == 'mot'){
        ...
        if($('input[name="updateMOT"]:checked').val()){
            ...
            if(eventDate != null && eventDate != '' && motAsPerEventDate != 'Invalid date' && moment(motAsPerPlanDate).format("DD MMM YYYY") != moment(motAsPerEventDate).format("DD MMM YYYY")){
                ...
                $('#current_mot_date').val(moment(motAsPerPlanDate).format("DD MMM YYYY"));
                $('#current_mot_date').attr('disabled', 'disabled');
                $('.current_mot_calendar').datepicker('setDate', moment(motAsPerPlanDate).format("DD MMM YYYY"));
