...
var telematics = new Vue({
    ...
    data: {
        ...
        icons: {
            ...
            asset_indication_pin: {
                icon: '/img/vehicle_images/asset_indication_pin.png'
            }
        },
        computed: {
    ...
    methods: {
        ...
        plotSearchedVehicleMap: function plotSearchedVehicleMap() {
            var vm = this;
            var value_search = $("#registrationTelematicsLive").val();
            ...
            if (filterType != "" || value_search != "" || userName != "" || region != "") {
                ...
                u.forEach(vm.vehicles_on_fleet, function (value) {
                    if ((region == '' || value['regionId'] == region) && (value_search == '' || value['registration'] == value_search) && (userName == '' || parseInt(value['driver_id']) == userName) && (filterType == '' || value['vehicleTypeId'] == filterType)) {
                        regsToKeep.push(value['registration']);
                    }
                });
            }
        ...
        showLocations: function showLocations() {
            ...
            this.$http.post('/telematics/getAllLocations').success(function (response) {
                $("#processingModal").modal('hide');
                if (!$('input[name=display_location').is(":checked")) {
                ...
                u.forEach(response, function (value) {
                    if ($('input[name=display_location').is(":checked")) {
                        locationMarkers.push([value.id, parseFloat(value.latitude), parseFloat(value.longitude)]);
                    }
                });
        ...
        assetPlotOnMap: function assetPlotOnMap() {
            var _this = this;
            var asset_on_fleet = [{
                'latitude': 51.484254,
                'longitude': -0.27498900000000004
            }, {
                'latitude': 52.68835,
                'longitude': -1.792332
            }, {
            ...
            var markers = [];
            var html = '';
            _this.$set('assets_on_fleet', asset_on_fleet);
            var tempAssetId = 1;
            u.forEach(this.assets_on_fleet, function (value) {
                //markers.push([html,value['latitude'],value['longitude'],value['vehicle_id'],value['markerType'],value['registration']]);
                markers.push([html, value['latitude'], value['longitude'], tempAssetId, 'asset_indication_pin', 'xxx']);
                tempAssetId++;
            });
...
$(document).ready(function () {
    ...
    $('.asset-pin-toggle').on("click", function () {
