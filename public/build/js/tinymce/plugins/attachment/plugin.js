...
/*jsc
["tinymce.plugins.attachment.Plugin","tinymce.core.PluginManager","tinymce.plugins.attachment.api.Commands","tinymce.plugins.attachment.core.FilterContent","tinymce.plugins.attachment.ui.Buttons","global!tinymce.util.Tools.resolve","tinymce.plugins.attachment.ui.Dialog","tinymce.core.util.Tools","global!Math","global!RegExp","tinymce.plugins.attachment.api.Settings","tinymce.plugins.attachment.core.Utils","tinymce.plugins.attachment.ui.AdvTab","tinymce.plugins.attachment.ui.MainTab","tinymce.plugins.attachment.ui.SizeManager","tinymce.plugins.attachment.ui.UploadTab","global!document","ephox.sand.api.FileReader","tinymce.core.util.Promise","tinymce.core.util.XHR","ephox.sand.api.URL","tinymce.core.ui.Factory","tinymce.plugins.attachment.core.Uploader","ephox.sand.util.Global","ephox.sand.api.XMLHttpRequest","global!window","ephox.katamari.api.Resolve","ephox.katamari.api.Global"]
jsc*/
...
define(
  'tinymce.plugins.attachment.api.Settings',
  [
  ],
  function () {
 ...
 */
define(
  'tinymce.plugins.attachment.core.Utils',
  [
    'global!Math',
    'global!document',
    'ephox.sand.api.FileReader',
...
define(
  'tinymce.plugins.attachment.ui.AdvTab',
...
// define(
//   'tinymce.plugins.attachment.ui.SizeManager',
...
// );
define(
  'tinymce.plugins.attachment.ui.MainTab',
  ...
  function (Tools, Settings, Utils, SizeManager) {
    ...
    var getGeneralItems = function (editor, imageListCtrl) {
      ...
      generalFormItems.push({ type: 'label', label: '', text: 'Maximum file size 10MB (.doc, .docx, .pdf, .xls, .xlsx, .ppt, .pptx).', classes: 'js-attachment' });
      if(!$("div#attachFile").length) {
        var inp = $('<div id="attachFile" style="display:none"></div>');
 ...
 */
define(
  'tinymce.plugins.attachment.core.Uploader',
  [
    'ephox.sand.api.XMLHttpRequest',
    'global!document',
    'global!window',
...
define(
  'tinymce.plugins.attachment.ui.UploadTab',
  ...
  function (URL, Factory, Settings, Utils, Uploader) {
    ...
    var acceptExts = '.doc, .docx, .pdf, .xls, .xlsx';
 ...
 */
define(
  'tinymce.plugins.attachment.ui.Dialog',
  [
    'global!Math',
    'global!RegExp',
    'tinymce.core.util.Tools',
    ...
    return function (editor) {
      ...
      function showDialog(imageList) {
        ...
        function onSubmitForm(e) {
          ...
          if(!data.src) {
            e.preventDefault();
            toastr["error"]('Please select a file to attach.');
            return;
          }
          ...
          if(data.src == null || data.src == '') {
            checkCancelButtonFlag = 1;
          } else {
            checkCancelButtonFlag = 2;
          }
        ...
        win = editor.windowManager.open({
          ...
          onClose: function() {
            ...
            //If user attach document and then click on cancel then we have to remove it.
            //Following code removes the attachment if user clicks on cancel button.
            setTimeout(function() {
              var tinymceEditor = $('.tab-pane.active').find('.js-tinymce-editor');
                if(checkCancelButtonFlag < 2) {
                  var removeElement = tinymceEditor.find('.mce-path-item').find('.mce-fieldset-title.d-none');
                  if(removeElement.length) {

                    //Re-calculate total size of uploaded attachments
                    var fileSize = removeElement.attr('data-file-size');
                    var totalFileSize = tinymceEditor.find('.mce-path-item').find('.js-total-file-size').val();
                    var updatedFileSize = parseInt(totalFileSize) - parseInt(fileSize);
                    tinymceEditor.find('.mce-path-item').find('.js-total-file-size').val(updatedFileSize);

                    //Remove attachment name and unique id from the list
                    var attachmentId = removeElement.find('.js-remove-attachment').attr('data-attachment-id');
                    var attachmentList = tinymceEditor.find('.js-attachment-list').val();
                    attachmentList = attachmentList.replace(attachmentId, '');
                    attachmentList = attachmentList.replace(/^,|,$/g, '');
                    tinymceEditor.find('.js-attachment-list').val(attachmentList);
                    removeElement.remove();
                  }
                } else {
                  if(tinymceEditor.find('.js-default-msg').length) {
                      tinymceEditor.find('.js-default-msg').remove();
                  }
                  tinymceEditor.find('.mce-path-item').find('.mce-fieldset-title').removeClass('d-none');
                }
                checkCancelButtonFlag = 0;
              }, 100);
                // $('div#attachFile').remove();
            },
          });
      }
...
define(
  'tinymce.plugins.attachment.api.Commands',
  [
    'tinymce.plugins.attachment.ui.Dialog'
  ],
  function (Dialog) {
...
define(
  'tinymce.plugins.attachment.core.FilterContent',
  [
    'tinymce.core.util.Tools'
  ],
  function (Tools) {
...
define(
  'tinymce.plugins.attachment.ui.Buttons',
  [
    'tinymce.plugins.attachment.ui.Dialog'
  ],
  function (Dialog) {
    var register = function (editor) {
      editor.addButton('attachment', {
        image: '../../../../img/attachment.svg',
        classes: 'js-attachment-icon',
        tooltip: 'Attach File',
...
define(
  'tinymce.plugins.attachment.Plugin',
  [
    'tinymce.core.PluginManager',
    'tinymce.plugins.attachment.api.Commands',
    'tinymce.plugins.attachment.core.FilterContent',
  ...
  function (PluginManager, Commands, FilterContent, Buttons) {
    PluginManager.add('attachment', function (editor) {
...
dem('tinymce.plugins.attachment.Plugin')();
})();
