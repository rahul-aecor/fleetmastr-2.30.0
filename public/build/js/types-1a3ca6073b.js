...
$(document).ready(function() {
    ...
    document.addEventListener('keydown', vehicleProfileSelectKeydownHandler, true);
...
function vehicleProfileSelectKeydownHandler(event){
    if(event.srcElement.id == 's2id_autogen1_search') {
        setTimeout(function() {
            var search = $('#s2id_autogen1_search').val();
...
$(document).ready(function() {
    ...
    $('#searchType').on('click', function(event) {
        ...
        if(searchFiler == '' || !searchFiler) {
            searchFiler = $('#s2id_autogen1_search').val();
        }
        if (searchFiler.length === 0) {
            grid[0].p.search = false;
            $.extend(grid[0].p.postData,{filters:""});
