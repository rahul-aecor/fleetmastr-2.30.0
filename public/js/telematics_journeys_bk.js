...
$("#journeyDateRangeFilter")
    ...
    .setStartDate(
        $("#commonDaterange").data("daterangepicker").startDate.format("DD/MM/YYYY")
    );
$("#journeyDateRangeFilter")
    .data("daterangepicker")
    .setEndDate(
        $("#commonDaterange").data("daterangepicker").endDate.format("DD/MM/YYYY")
    );
...
function getJourneyFilterData() {
    ...
    data = {
        ...
        startDate: $("#journeyDateRangeFilter")
            .data("daterangepicker")
            .startDate.format("YYYY-MM-DD"),
        endDate: $("#journeyDateRangeFilter")
            .data("daterangepicker")
...
function initializeDriverAnalysisData(data) {
    ...
    $(journeyData).each(function(k, journey) {
        labels.push('');
        maxSpeedData.push(journey.speed_limit != null ? parseFloat(journey.speed_limit * 2.236936).toFixed(2) : 0);
        vehicleSpeedData.push(journey.speed != null ? parseFloat(journey.speed * 2.236936).toFixed(2) : 0);
...
jQuery("#journeyJqGrid").jqGrid({
    ...
    colModel: [{
        ...
        { label: "User", name: "user", title: false},
        {
            label: "Start",
        ...
        },
        {
            ...
            formatter: function(cellvalue, options, rowObject) {
                ...
                if (cellvalue != null && cellvalue != "00:00") {
                    return idlingTimeFomat(cellvalue);
                }
                return "";
            },
        },
        ...
        },
        {
            ...
            formatter: function(cellvalue, options, rowObject) {
                if (cellvalue != null) {
                    return parseFloat(cellvalue * 0.00062137).toFixed(2);
                }
                return 0;
            },
            unformat(cellvalue, options, cell) {
        ...
        },
        {
            ...
            formatter: function(cellvalue, options, rowObject) {
                if (cellvalue != null) {
                    return parseFloat(cellvalue * 2.236936).toFixed(2);
                }
                return '-';
            },
            unformat(cellvalue, options, cell) {
        ...
        },
        {
            ...
            sorttype: "number",
            formatter: function(cellvalue, options, rowObject) {
                $odometerStartMilesPerHour = rowObject.journeyStart / 1609.344;
                if (rowObject.journeyStart != null) {
                    return numberWithCommas($odometerStartMilesPerHour.toFixed(0));
                } else {
                    return '-';
                }
            },
        },
        {
            ...
            sorttype: "number",
            formatter: function(cellvalue, options, rowObject) {
                $odometerEndMilesPerHour = rowObject.journeyEnd / 1609.344;
                if (rowObject.journeyEnd != null) {
                    return numberWithCommas($odometerEndMilesPerHour.toFixed(0));
                } else {
                    return '-';
                }
            },
        },
        {
            ...
            sorttype: "number",
            formatter: function(cellvalue, options, rowObject) {
                var gallons = parseFloat(rowObject.fuel * 0.264172);
                var miles = parseFloat(rowObject.gps_distance * 0.00062137);
                var mpg = 0;
                if (gallons != null && gallons != "undefined" && gallons != 0) {
                    mpg = parseFloat(miles / gallons).toFixed(2);
                }
                var lab;
...
$("#journeyJqGrid").navButtonAdd("#journeyJqGridPager", {
    ...
    onClickButton: function() {
        ...
        // filters = JSON.stringify(getJourneyFilterData());
        var fileterData = getJourneyFilterData();
    ...
    // };
        ...
        var data = {groupOp:"and",rules:[]};
        if(fileterData.userFilterValue) {
            data.rules.push({
                field:"j.user_id",
                op:"eq",
                data: fileterData.userFilterValue
            });
        }

        if(fileterData.registrationFilterValue) {
            data.rules.push({
                field:"j.vrn",
                op:"eq",
                data: fileterData.registrationFilterValue
            });
        }

        if(fileterData.regionFilterValue) {
            data.rules.push({
                field:"v.vehicle_region_id",
                op:"eq",
                data: fileterData.regionFilterValue
            });
        }

        if(fileterData.startDate) {
            data.rules.push({
                field:"start_time",
                op:"ge",
                data: fileterData.startDate+" 00:00:00"
            });
            data.rules.push({
                field:"start_time",
                op:"lt",
                data: fileterData.endDate+" 23:59:59"
            });
        }

        if(fileterData.postcode) {
            data.rules.push({
                field:"jd.post_code",
                op:"eq",
                data: fileterData.postcode
            });
        }
            ...
            $.validator.addMethod(
                ...
                function(value, element) {
                    return (
                        ...
                        /^((([A-PR-UWYZ][0-9])|([A-PR-UWYZ][0-9][0-9])|([A-PR-UWYZ][A-HK-Y][0-9])|([A-PR-UWYZ][A-HK-Y][0-9][0-9])|([A-PR-UWYZ][0-9][A-HJKSTUW])|([A-PR-UWYZ][A-HK-Y][0-9][ABEHMNPRVWXY]))\s?([0-9][ABD-HJLNP-UW-Z]{2})|(GIR)\s?(0AA))|((?:^[AC-FHKNPRTV-Y][0-9]{2}|D6W)[ -]?[0-9AC-FHKNPRTV-Y]{4})$/i.test(
                            value
                        )
                    );
                },
