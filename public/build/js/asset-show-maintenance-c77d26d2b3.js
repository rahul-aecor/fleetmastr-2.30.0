    ...
    $('.maintenance_event_name').editable({
        ...
        success: function (response) {
            if(response.status) {
                ...
                $("#view_all_events").html(response.dataAll.tBody);
                initEditable();
    ...
    $.ajax({
        ...
        success:function(response){
            if(response.status) {
                ...
                $("#view_all_events").html(response.tBodyAll);
            } else {
                $("#eventName").parent().addClass('has-error');
    ...
    colModel: [
        ...
        },
        {
            ...
            formatter: function( cellvalue, options, rowObject ) {
                ...
                returnString += '<a title="Details" href="javascript:void(0);" class="btn btn-xs grey-gallery tras_btn show-maintenance-history" data-maintenance-history-id="'+ rowObject.id +'"><i class="jv-icon jv-find-doc text-decoration icon-big"></i></a><a href="javascript:void(0);" title="Edit" class="btn btn-xs grey-gallery edit_maintenance_history tras_btn" data-maintenance-history-edit-id="'+ rowObject.id +'"><i class="jv-icon jv-edit icon-big"></i></a>';
                if(rowObject.event_status == 'Complete') {
                    returnString += '<a title="Delete" href="javascript:void(0);" class="btn btn-xs grey-gallery tras_btn delete-maintenance-history" data-maintenance-history-delete-id="'+ rowObject.id +'"><i class="jv-icon jv-dustbin icon-big"></i></a>';
                }
                return returnString;
            }
        }
...
$(document).on('click', '#edit_maintenance_history_save', function(e){
    ...
    $.validator.addMethod("checkEventDateAndStatus", function(value, element) {
        return !(editAssetMaintenanceStatus == 'Incomplete' && editAssetMaintenanceEventDate != '');
            ...
            'edit_maintenance_planned_date': {
                required: {
                    depends: function(element) {
                        ...
                        return $('#edit_maintenance_planned_date').val() == '' ? true : false;
                    }
                },
            },
...
$(document).on('click', '#editAssetMaintenancePlannedDateLockBtn', function(){
    if($("#edit_maintenance_status").val()=="Incomplete"){
        ...
        if(isAnyChange==true){
            ...
            $("#editAssetMaintenancePlannedDateLockBtn > i").removeClass("jv-lock").addClass("jv-calendar");
        }
    }
});
