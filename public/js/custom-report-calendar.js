...
function initReportDateTimeRangePicker(ranges, maxDate, timePicker = false, slug = null) {
	if($('input[name="date_range"]').length) {
		...
		if(slug == 'standard_weekly_maintanance_report') {
...
window.addEventListener('apply.daterangepicker', function (ev) {
	...
	} else if($('#reportSlug').val() == 'standard_weekly_maintanance_report') {
		if (endDate.diff(startDate, 'days') > 185) {
			$('#date_range').val('');
			toastr["error"]('The maximum date range selection is 186 days');
			$('#date_range').trigger('click');
		}
	} else {
